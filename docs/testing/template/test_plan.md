...
| TC-tpl-014 | Create | Create unified v1 step template | `harness_create(resource_type="template_v1", org_id="AI_Devops", project_id="AICHAT", body={template_yaml: "version: 1\ntemplate:\n  name: npm_build\n  identifier: npm_build\n  step:\n    run:\n      container:\n        image: node:20\n      script: npm run build", label: "1.0.0", is_stable: true})` | Unified v1 template created (JSON body matches UI: `template_yaml` includes `version: 1`, MCP also sends `yaml_version: "1"`) |
...
- **template_v1** create/update use v1 REST: `POST /v1/.../templates` and `PUT /v1/.../templates/{id}/versions/{version}` with JSON body (`template_yaml`, `yaml_version: "1"`, `identifier`, `label`, `name`, `git_details.store_type: INLINE`). Unified v1 `template_yaml` must include top-level `version: 1` plus `template.step` / `stage` / `pipeline` (not NG `type`/`spec`). MCP always sets `yaml_version: "1"` in the JSON body as well (same as Harness UI).
